...
def validate_skill(skill_dir: Path, expected_name: str, errors: list[str]) -> None:
    skill_md = skill_dir / "SKILL.md"
    if not skill_md.exists():
        errors.append(f"Missing SKILL.md: {skill_md.relative_to(REPO_ROOT)}")
        return
    ...
    if not name:
        errors.append(f"{skill_md.relative_to(REPO_ROOT)}: missing required field 'name'")
    elif len(name) > 64:
        errors.append(f"{skill_md.relative_to(REPO_ROOT)}: name exceeds 64 characters")
    elif not NAME_PATTERN.fullmatch(name):
        errors.append(f"{skill_md.relative_to(REPO_ROOT)}: name must match ^[a-z0-9]+(?:-[a-z0-9]+)*$")

    if name and name != expected_name:
        errors.append(
            f"{skill_md.relative_to(REPO_ROOT)}: name '{name}' must match parent directory '{expected_name}'"
        )

    if not description:
        errors.append(f"{skill_md.relative_to(REPO_ROOT)}: missing required field 'description'")
    elif len(description) > 1024:
        errors.append(f"{skill_md.relative_to(REPO_ROOT)}: description exceeds 1024 characters")


def main() -> int:
    if not SKILLS_LAYER_PATH.exists():
        print(f"Missing skills layer config: {SKILLS_LAYER_PATH.relative_to(REPO_ROOT)}")
        return 1

    if not SKILLS_ROOT.exists():
        print(f"Missing skills directory: {SKILLS_ROOT.relative_to(REPO_ROOT)}")
        return 1
    ...
    for expected in sorted(expected_dirs):
        validate_skill(SKILLS_ROOT / expected, expected, errors)
