...
# Role header is trusted only in dev mode; production must use verified identity
_ALLOW_ROLE_HEADER = _to_bool(os.getenv("RAG_API_ALLOW_ROLE_HEADER"), default=True)
...
def _resolve_caller_role(header_value: str | None) -> str:
    """Resolve caller role: header only in dev mode, otherwise use default."""
    if _ALLOW_ROLE_HEADER and header_value:
        return header_value
    return settings.default_caller_role
...
def skills_plan(
    ...
    caller_role = _resolve_caller_role(x_caller_role)
    try:
