...
def merge_allergy_adverse_event(tx, event: dict[str, Any], payload: dict[str, Any]) -> None:
    allergen = payload.get("allergen")
    if not allergen:
        return
    tx.run(
        """
        ...
        MERGE (ae:AdverseEvent {id: $adverse_event_id})
          ON CREATE SET ae.symptom_name = $symptom,
                        ...
                        ae.meddra_term = $diagnosis,
                        ae.allergen = $allergen,
                        ae.event_family = 'ALLERGY_INTOLERANCE'
        MERGE (p)-[:REPORTED_ADVERSE_REACTION]->(ae)
        MERGE (ae)-[:TRIGGERED_BY_EVENT]->(ce)
        WITH ae
        CALL {
          WITH ae
          WITH ae WHERE $allergen_is_medication
          MATCH (m:Medication {name: $allergen})
          MERGE (ae)-[:ASSOCIATED_WITH_MEDICATION]->(m)
          RETURN count(*) AS _
        }
        RETURN ae
        """,
        {
            ...
            "diagnosis": payload.get("diagnosis"),
            "allergen": allergen,
            "allergen_is_medication": allergen in {
                "Penicillin", "Aspirin", "ACE inhibitors", "Morphine",
                "Sulfa drugs", "Amoxicillin",
            },
            "severity": payload.get("reaction_severity", "unknown"),
            "adverse_event_id": f"ae-allergy-{event['event_id']}",
            "event_ts": event["event_ts"],
            "source_event_id": event["event_id"],
        },
    )